...
> = {
  ...
  REJECTED: {
    label: "Rejected",
    className:
      "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300/40",
    icon: <XCircle className="h-3 w-3" />,
  },
  OVERDUE: {
    label: "Overdue",
    className:
      "bg-red-100 dark:bg-red-950/60 text-red-800 dark:text-red-300 border border-red-300/60 font-bold",
...
export default function AdminBorrowsPage() {
  ...
  const { mutate: checkOverdue, isPending: isCheckingOverdue } =
    useCheckOverdueBorrows();
  ...
  const [selectedBorrowForReturn, setSelectedBorrowForReturn] =
    useState<IBorrow | null>(null);
  const [fineInput, setFineInput] = useState<number>(0);
  const [deleteBorrowId, setDeleteBorrowId] = useState<
    number | string | null
  >(null);
  ...
  const handleStatusFilterChange = (val: BorrowStatus | "ALL") => {
    setStatusFilter(val);
    setCurrentPage(1);
  };
  ...
  const filtered = borrows.filter((b) => {
    ...
    return (
      (titleMatch || userNameMatch || userEmailMatch || idMatch) && statusMatch
    );
  });
  ...
  const handleConfirmReturn = () => {
    ...
    returnBook(
      {
        borrowId: selectedBorrowForReturn.id,
        fineAmount: Number(fineInput) || 0,
      },
      {
        onSuccess: () => {
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Review borrow requests, verify membership, issue approvals with
            auto-calculated due dates, and run overdue audits.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        ...
        <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-4">
          ...
          <p className="text-2xl font-extrabold text-red-800 dark:text-red-200 mt-1">
            {overdueList.length}
          </p>
        </div>
                      ...
                      >
                        ...
                        <td className="px-4 py-4 max-w-[220px]">
                          <p className="font-bold text-foreground line-clamp-1">
                            {b.book?.title ?? `Book #${b.bookId}`}
                          </p>
                          <p className="text-[11px] text-muted-foreground">
                            {pages} Pages ({calculatedDays}d rate)
                          </p>
                          {b.book?.locationCell && (
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold ${cfg.className}`}
                          >
                            {cfg.icon} {cfg.label}
                          </span>
                          {b.status === "OVERDUE" && (
                            <p className="text-[10px] text-red-600 font-bold mt-1">
                              {overdueDays}d overdue (Fine: ৳
                          ...
                          {b.status === "RETURNED" && b.returnedBy && (
                            <p className="text-[10px] text-blue-600 dark:text-blue-400 font-medium mt-1 flex items-center gap-1">
                              ...
                              By: {b.returnedBy.name} (
                              {b.returnedBy.role.replace("_", " ")})
                            </p>
                          )}
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in-50">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl space-y-4 animate-in zoom-in-95">
            ...
            <div className="space-y-2 text-xs">
              <p>
                <strong>Book:</strong> {selectedBorrowForReturn.book?.title}
              </p>
              <p>
                <strong>Borrower:</strong> {selectedBorrowForReturn.user?.name}{" "}
                ({selectedBorrowForReturn.user?.email})
              </p>
              <p>
                <strong>Due Date:</strong>{" "}
                {selectedBorrowForReturn.dueDate
                  ? new Date(selectedBorrowForReturn.dueDate).toLocaleDateString()
                  : "N/A"}
              </p>
              {calcDaysOverdue(selectedBorrowForReturn.dueDate) > 0 && (
                <p className="text-red-600 font-bold">
                  Overdue by{" "}
                  {calcDaysOverdue(selectedBorrowForReturn.dueDate)} days
                </p>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in-50">
          <div className="relative w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-2xl text-card-foreground text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                This administrative action will permanently remove this loan
                record from the circulation archive.
              </p>
            </div>
